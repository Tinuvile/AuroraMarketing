...
-- 导出  表 aurora-marketing-01.raffle_activity_account_flow 结构
CREATE TABLE IF NOT EXISTS `raffle_activity_account_flow` (
...
-- 正在导出表  aurora-marketing-01.raffle_activity_account_flow 的数据：~0 rows (大约)

-- 导出  表 aurora-marketing-01.raffle_activity_order 结构
CREATE TABLE IF NOT EXISTS `raffle_activity_order` (
  ...
  `status` varchar(8) NOT NULL COMMENT '订单状态（not_used、used、expire）',
  `create_time` datetime NOT NULL DEFAULT (now()) COMMENT '创建时间',
  `update_time` datetime NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
-- 正在导出表  aurora-marketing-01.raffle_activity_order 的数据：~0 rows (大约)
