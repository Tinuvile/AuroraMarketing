...
public class RaffleActivityController implements IRaffleActivityService {
     ...
     * </p>
     *
     *   curl -X POST http://localhost:8091/api/v1/raffle/activity/is_calendar_sign_rebate -d "userId=tinuvile" -H "Content-Type: application/x-www-form-urlencoded"
     */
    @RequestMapping(value = "is_calendar_sign_rebate", method = RequestMethod.POST)
    @Override
    public Response<Boolean> isCalendarSignRebate(@RequestParam String userId) {
        try {
            log.info("查询用户是否完成日志签到返利开始 userId:{}", userId);
    ...
    public Response<Boolean> creditPayExchangeSku(SkuProductShopCartRequestDTO request) {
        try {
            ...
            // 创建兑换商品sku订单，outBusinessNo 每次创建出一个单号。
